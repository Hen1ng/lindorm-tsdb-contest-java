...
public class RowFactory  implements Serializable{
        ...
        public int[] ints = new int[45];

        public double[] doubles = new double[9];
        public String[] byteBuffers = new String[6];
        ...
        Row GetRow(){
            ...
            for(int i=45;i<54;i++){
                columns.put(columnsName[i],new ColumnValue.DoubleFloatColumn(doubles[i-45]));
            }
            for(int i=54;i<60;i++){
                int length = random.nextInt(25);
                ByteBuffer byteBuffer = ByteBuffer.wrap(byteBuffers[i-54].getBytes());
